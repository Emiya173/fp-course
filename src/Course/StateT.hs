...
import Control.Arrow qualified as B
...
instance (Functor k) => Functor (StateT s k) where
  (<$>) ::
    ...
    StateT s k a ->
    StateT s k b
  (<$>) f (StateT x) = StateT $ \s -> Bi.first f <$> x s
...
instance (Monad k) => Applicative (StateT s k) where
  ...
  (<*>) ::
    ...
    StateT s k a ->
    StateT s k b
  (<*>) (StateT f) (StateT a) = StateT $ \s -> f s >>= \(g, s') -> Bi.first g <$> (a s')
...
distinctF xs = evalT (filtering f xs) S.empty
  where
    f x
      ...
      | otherwise = StateT (\s -> pure (S.notMember x s, S.insert x s))
