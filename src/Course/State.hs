...
instance Applicative (State s) where
  ...
  (<*>) ::
    ...
    State s a ->
    State s b
  (<*>) (State f) a = State $ \s -> let (x, s') = f s in runState (x <$> a) s'
...
instance Monad (State s) where
  (=<<) ::
    ...
    State s a ->
    State s b
  (=<<) f (State g) = State $ \s -> let (a, s') = g s in runState (f a) s'
...
firstRepeat as = eval (findM f as) S.empty
  where
    f x = State $ \s -> (S.member x s, S.insert x s)
